...
public class CommandFactoryLoader {
    ...
    private static void loadCommandsFromConfig(CommandFactory factory) {
        try {
            ...
            if (configStream == null) {
                CalculatorLogger.warning("Commands config file not found");
                return;
            }
            ...
            CalculatorLogger.severe("Failed to load commands config: " + e.getMessage());
        }
    }
    ...
    private static void loadCommandsFromJar(CommandFactory factory, String jarPath) {
        try {
            ...
            if (!jarFile.exists()) {
                CalculatorLogger.warning("JAR file not found: " + jarPath);
                return;
            }
                ...
                if (entryName.endsWith(".class")) {
                    ...
                    try {
                        ...
                        if (Command.class.isAssignableFrom(clazz) && 
                            ...
                            CalculatorLogger.info("Loaded command: " + info.name() + 
                                " from " + jarPath);
                        }
                    } catch (ClassNotFoundException e) {
                        CalculatorLogger.warning("Failed to load class: " + className);
                    }
                }
            }
            ...
            CalculatorLogger.severe("Failed to load JAR: " + jarPath);
        }
    }
    ...
    private static void loadDefaultCommands(CommandFactory factory) {
        ...
        CalculatorLogger.info("Loaded default commands");
    }
}